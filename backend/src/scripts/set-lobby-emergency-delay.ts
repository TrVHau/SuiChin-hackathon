...
async function main() {
  ...
  if (!env.ADMIN_SECRET_KEY) {
    throw new Error(
      "ADMIN_SECRET_KEY is required because the admin wallet must hold LobbyAdminCap",
    );
  }
  ...
  tx.moveCall({
    target: `${packageId}::nft_valuation_lobby_config::set_emergency_refund_delay`,
    arguments: [
      tx.object(env.LOBBY_CONFIG_OBJECT_ID),
      tx.object(adminCapId),
      tx.pure.u64(env.LOBBY_EMERGENCY_REFUND_DELAY_MS),
    ],
  });
    ...
    JSON.stringify(
      {
        ...
        emergencyRefundDelayMs: env.LOBBY_EMERGENCY_REFUND_DELAY_MS,
      },
      null,
      2,
    ),
